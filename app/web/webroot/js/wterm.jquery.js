  ...
  $.fn.wterm = function( options ) {
    ...
    return this.each( function() {
      ...
      if( typeof settings.WIDTH == 'string' || typeof settings.HEIGHT == 'string'){
        if( settings.WIDTH && settings.HEIGHT ) element.css( { width: eval(settings.WIDTH), height: eval(settings.HEIGHT) } );
      } else {
      ...
      **/
      var update_content = function( p, cmd, data ) {
        if(data){
          ...
          content.append( '<div><span>' + p + ' ' + cmd + '</span><div><pre>' + data + '</pre></div></div>' );
        }else{
          data = '';
	  ...
	  settings.update_content=update_content;
      ...
      **/
      input_form.submit( function( e ) {
        if(settings.beforeSubmit != undefined){
        ...
        var _dispatch = function( key, tokens ) {
            // njh: add on submit functionality here

            if( typeof key === 'function' ) {
                // for baked-in functionality
                data = key( tokens );
                ...
                if(settings.targets.length === 0){
                    update_content( get_current_prompt(), value, "No targets selected.");
                }
                $.each(settings.targets, function(target_index, target_location){
                    var to_send = { };
                    to_send[ settings.AJAX_PARAM ] = tokens.join( ' ' );
                    to_send['target'] = target_location;
                    
                    // add any extra ajax request thingies
                    $.each(settings.AJAX_EXTRAS, function(index, item){
                        to_send[index] = item;
                    });

                    console.log("Sending command: ",to_send);

                    var on_complete = function( data, text_status ) {
                      if(data == ''){
                        update_content( get_current_prompt(), value, data );
                      } else {
                        update_content( get_current_prompt(), value, 'From "'+settings.targetNames[target_index]+':"\r\n'+data );
                      }
                    };
        ...
        } else if( settings.AJAX_URL != '' ) {
            // send it to the AJAX URL automagically and wait for the response
            _dispatch(settings.AJAX_URL, tokens);
